...
const CATALOG = [
  ...
  // ---- pipeline stages ----
  // These are the steps a document or a question actually passes through. Drawn
  // as one "worker" they hide every decision that matters: what parses a scanned
  // PDF, where the chunk boundaries fall, what happens when the model returns a
  ...
  // ------------------------------------------------------- Migration & legacy
  // Replacing a system that is still running is most of the work, and none of it
  // could be drawn: not the thing being replaced, not the facade in front of both,
  // not the job that repairs the difference between them.
  {
    type: 'legacy_system',
    label: 'Legacy System',
    category: 'Integration',
